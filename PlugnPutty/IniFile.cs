...
public class IniFile
{
	...
	public void Load(string file_name, bool merge)
	{
		...
		Log.Instance.Debug(string.Format("Loading: {0}", file_name));
		...
		while (!stream_reader.EndOfStream)
		{
			...
			if (line != string.Empty)
			{
				...
				if (regex_comment.Match(line).Success)
				{
					...
					Log.Instance.Debug(string.Format("Skipping comment: {0}", match.Groups[0].Value));
				}
				else if (regex_section.Match(line).Success)
				{
					// Section
					match = regex_section.Match(line);
					Log.Instance.Debug(string.Format("Section [{0}]", match.Groups[1].Value));
				...
				{
					Log.Instance.Debug(string.Format("unknown type of data: {0}", line));
				}
			}
		}
		...
		Log.Instance.Debug(string.Format("{0}: loaded", file_name));
	}
	...
	public void Save(string file_name)
	{
		...
		Log.Instance.Debug(string.Format("Saving: {0}", file_name));

		stream_writer = new StreamWriter(file_name, false);
		foreach (IniSection s in SectionsName)
		{
			Log.Instance.Debug(string.Format("Section: [{0}]", s.Name));
			...
			foreach (IniSection.IniKey k in s.Keys)
			{
				if (k.Comment == null)
				{
					if (k.Quotes == true)
					{
						Log.Instance.Debug(string.Format("Key: {0}={1}", k.Name, k.Value));
				...
				else
				{
					if (k.Quotes == true)
					{
						Log.Instance.Debug(string.Format("Key: {0}=\"{1}\";{2}", k.Name, k.Value, k.Comment));
		...
		Log.Instance.Debug(string.Format("{0}: saved", file_name));
	}
